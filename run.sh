...
if [ -f ${GF_PATHS_CONFIG} ]; then
  rm -rf ${GF_PATHS_CONFIG}
fi

if [ ! -f /var/lib/grafana/grafana.ini ]; then
  cp /root/grafana.ini /var/lib/grafana/grafana.ini
fi

ln -s /var/lib/grafana/grafana.ini /etc/grafana/grafana.ini 
